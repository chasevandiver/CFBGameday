...
| `newHcIntercept` / `newHcSlope` | 0 / 0 | **Identity** — the pooled fit stayed unconverged (its class is the double-count) |
| `newHcHealthyIntercept` | **−6** | Fitted `--tune-coaching-split` (2026.6.0): interior on both windows, holdout-supported, weeks-5+ improved. Outer clamp widened −4 → −8 to admit it |
...
receipts." 71 of the 91 Week 1 receipts had `vegas_spread = null`. Every one
of those games had DraftKings and Bovada lines on file since August.

**Root cause.** PostgREST returns at most 1,000 rows per request and says
nothing when it does. The Thursday freeze read every `line_snapshots` row for
the week's games in one request. Week 0: 8 games, ~560 rows, fine. Week 1:
91 games, 4,423 rows at 13:07 UTC on Sep 3, first 1,000 returned — which
happened to cover exactly the 20 games that got a line. The Sunday grader's
closing-line read had the same shape, which is why only 3 of the 8 Week 1
finals had a close.

**Fix.** `pageAll` (`src/lib/page-all.ts`, lifted from `salience-data.ts`
where it was private) on every read that can grow past the ceiling: the
freeze's snapshots, ratings and system ratings; the grader's snapshots and
scoring plays; the season-wide games read behind `gradeSeasonFinals`; the
game page's team-ATS snapshots; and the whole-season predictions reads on
Receipts and Model stats (~900 rows by December). The test fake learned
`order` and `range` so the grading tests run through the paging path.

**Backfill (owner: "Fix and backfill").** `recoverFreezeLine` rebuilds what
the freeze would have written from the snapshots captured before each row's
`created_at`, with the freeze's own functions — `consensusFromSnapshots`
under the freeze cutoff, `paramsForWeek`, `normalCdf`, `modelClv` — and
writes only the market fields: line, opener, edge, flag, cover probability,
and CLV where the grader had already stored a close. The model's spread,
total and win probability are exactly as frozen. Each row carries
`adjustments.line_recovered = {at, reason: "FREEZE-3", from: "line_snapshots"}`;
Receipts prints a dagger beside the line and a footnote under the week.
**Applied to the live project 2026-09-04 ~06:53 UTC** via SQL generated by
that function from the live inputs: 71 rows, 0 left unlined, 0 model fields
touched, verified by reading back. `scripts/recover-freeze-lines.ts` is the
same operation as an idempotent script for next time.

Two honest residuals, tracked as FREEZE-3b/3c: `consensus_flag` on the 71
rows stays false (the systems comparison never ran without a line), and two
week-bounded reads are deliberately left unpaged.

**What the recovered receipts say.** Week 1's flagged edges went from 6 to
73 of 91, so the Receipts strip and `/model/stats` now grade the whole slate
rather than a fifth of it. The four recovered finals that had no close yet
pick up CLV on the next grader pass, since `close_spread` still marks them
ungraded.

**Verified.** Vitest **2,091 across 142 files** (4 on `pageAll`, 6 on
recovery), typecheck, lint, `next build` green. No migration — `adjustments`
was already jsonb.
...
The pending row below closed the same session: ship. `newHcHealthyIntercept`
enters `DEFAULT_PARAMS` at the fitted **−6**, applied when the pre-coaching
composed preseason rating is ≥ 0 (the production analog of the pre-adjustment
chained prior the tuner classified on); the outer clamp widens −4 → −8
because a floor inside the fitted value would silently re-reject the
parameter the gate just passed. The pooled intercept and slope stay at their
rejected zeros — the healthy term stacks on top of them, so the pooled tuner
now fits residual rather than re-litigating the split. Tests pin the
boundary (≥ 0 charges, −0.01 doesn't, null prior takes the conservative
branch), the stack, and the widened clamp.

19 teams move in 2026, roughly −6 each: Utah (#7 → ~#19), Ole Miss,
Penn State, LSU, Michigan, Kansas State, Florida, Arkansas, Iowa State,
Kentucky, South Florida, Toledo, Washington State, James Madison, North
Texas, Memphis, Tulane, UConn, and the boundary case Kentucky at prev +0.2.
Applied to production via `preseason-refresh` (rebuild + reload + verify)
before the Thursday freeze; the frozen Week-0/1 receipts grade the call from
Saturday. Known residual, recorded rather than hidden: the backtest's main
report chains priors WITHOUT the coaching term (it always has — moot while
the params were identity zeros), so the report now slightly understates the
shipped model until the chain learns the term; the tuner that earned the
value measured the exact term it shipped.

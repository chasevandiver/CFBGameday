...
- **Team-specific home field:** each team's historical HFA blended 50/50 with the FBS average (~2.3). **[v2]** Computed by a one-time backfill job over 2015–2024 home/away margin residuals. *(Amended 2026-08-18: `teamHfaBlend = 0.5` has never been validated by any replay, and the reason is structural — production priced with the per-team table while the replay priced with a flat scalar, so audit 03:M-1's ~+1.9 inflation in that table could not appear in any calibration report. `replaySeason` now accepts a per-team map and `backtest.ts --tune-team-hfa` carries the pre-registered rule, whose Gate 0 is a split-half **identification** test rather than an accuracy bar. The standing rule if it fails is `teamHfaBlend = 0`; see 02:M-05 / 03:M-1v in docs/STATUS.md.)*
...
- Backtest the full pipeline before launch. Tune: K-factor, prior decay, margin cap, HFA blend, regression weights, margin σ. *(Amended 2026-08-18: the window was 2023–2025 and is now **2015–2025 by default**, a parsed flag rather than a constant — `--seasons=2023-2025` still reaches the old one. It widened because the archive backfill (BF-4) measured CFBD line coverage at **95–100% per season back to 2015** against a 55–70% assumption, and every tuner scores against a stored spread. Nine scored seasons where there were two: 2015 is the SP+-seeded warm-up and **2020 is chain-only** — replayed so the prior chain into 2021 is unbroken, scored by nothing, because empty stadiums collapse home-field advantage league-wide and the Pac-12 played no non-conference games at all. Two obligations come with the width and are not optional: every published number carries its **window label**, since a metric and the corpus it was computed on are one fact; and every fit reports **per-era** as well as pooled results, because a parameter pooled over eleven seasons is fitted partly on a sport that no longer exists — no portal, no NIL, an intact Pac-12, a four-team playoff. Era boundaries are drawn on rule changes and fixed in `scripts/lib/eras.ts` before any result is seen.)*
- **[v2] Bootstrap rule:** seed the earliest backtest season's priors from CFBD historical SP+. **Lookahead-bias guard is a hard requirement:** week-N predictions may only consume ratings/data available before week N. The backtest validates model parameters and calibration vs *stored* lines — it does **not** validate CLV or line movement (no historical intraday movement data exists anywhere; dropped from backtest scope — and eleven seasons of a window that has none is still none).

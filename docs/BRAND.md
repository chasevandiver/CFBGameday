# The Slate — Brand System & Visual Identity
...
**Renamed 2026-08-15: the product is `The Slate`, not `The CFB Slate`.** Owner
decision, and the reason is in §1 already — this has carried the NFL since
v1.0, so a name that says CFB describes half of it. Nothing else about the
identity moves: the mark, the palette, the type and every rule below are
unchanged, and the icon artwork was not touched. The wordmark string is the
whole diff, in `app/manifest.ts`, `app/layout.tsx`, `AppNav`, the OG cards, the
share card, the service worker and the iOS splash (regenerated by
`npm run brand`). Sections below quoting the old string are updated in place
rather than annotated; the old name survives in the changelog and the audits,
which are history.
...
The Slate is a private, serious sports intelligence and betting/pick'em
...
The slate of games. Every Saturday the user opens The Slate and sees the
...
Do not add: "CFB", "NFL", "The Slate", "Bet", "$", odds, team logos, helmet
...
extremely subtle green aura, the S/football mark centred, `THE SLATE` in
...
Through `app/manifest.ts`: name `The Slate`, short name `The Slate`,
...
edge, Slate branding — the icon appearing as a small brand stamp.
...
Wordmark: `THE SLATE` in Graduate, stacked or single-line by context. The
...
The Slate is not a sportsbook. It is the serious user's football slate —

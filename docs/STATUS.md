...
- [x] **OPS-4b — the cancelled-run fix never fired, and two more rows piled up
      ...
      **0076 settles 217 and 242** on 0073's rules exactly (3-hour cutoff, null
      `finished_at`, no DELETE). ⚠️ **Written but NOT applied** — the apply was
      refused by this session's sandbox, so it is the one piece of this row an
      owner still has to run. **Ordering is free** (two rows in an
      observability table, no constraint change, nothing reads them), so it can
      go before or after the deploy. Until it runs, `/admin` shows two amber
      "never finished" `scoreboard-loop` tiles, which is the card working. The
      code half needs no migration: `job_runs_status_check` already accepts
      `canceled` (0073).

...
/**
 * Game-card redesign, frontend-design skill pass — compare with
 * /slate/card-redesign. Everything the original brief fixed is unchanged;
 * the skill's "one risk" budget is spent on a signature element: the sweat
 * meter, a turf field strip on every live card with the ball at its true
 * yard line, drive direction, team-colored end zones, and the red zone
 * shaded when active. It turns "1st & Goal at OSU 4" into a picture —
 * danger reads spatially before any text is parsed. The cover verdict also
 * goes italic condensed, the way every broadcast score bug sets it.
 *
...
interface LiveGame {
  ...
  possFlipped: boolean;
  /** ball position for the sweat meter: 0 = away goal line, 100 = home goal line */
  ballX: number;
  /** drive direction on the strip (home defends the right end zone) */
  dir: "left" | "right";
  lastPlay: string;
  pick: Pick;
  crewWith: CrewMate[];
...
/* =====================================================================
   Sweat meter — the signature element. The whole field in a thin strip:
   team-colored end zones, the ball at its true yard line, an arrow for
   drive direction, and the red zone shaded when the ball is inside it.
   "1st & Goal at OSU 4" as a picture instead of a sentence.
   ===================================================================== */

function FieldStrip({ g }: { g: LiveGame }) {
  return (
    <div className="gc-field" aria-hidden>
      <span className="gc-ez gc-ez-l" style={{ background: g.away.color }} />
      <span className="gc-ez gc-ez-r" style={{ background: g.home.color }} />
      {g.redZone && <span className={`gc-rzband ${g.dir === "right" ? "gc-rzband-r" : "gc-rzband-l"}`} />}
      <span className="gc-fieldball" style={{ left: `${g.ballX}%` }}>
        {g.dir === "left" && <span className="gc-dir">◂</span>}
        <Ball />
        {g.dir === "right" && <span className="gc-dir">▸</span>}
      </span>
    </div>
  );
}
...
/* ---- sweat meter: the field as a strip ---- */
.gc-field {
  position: relative;
  height: 14px;
  margin: 7px 16px 0;
  border-radius: 4px;
  overflow: hidden;
  /* turf: the one quiet green on the card, dark enough to never read as "winning" */
  background:
    repeating-linear-gradient(90deg, transparent 0 calc(10% - 1px), rgba(244, 239, 230, 0.10) calc(10% - 1px) 10%),
    linear-gradient(180deg, #232b20, #1c231a);
}
.gc-ez {
  position: absolute;
  top: 0;
...
.gc-rzband {
  ...
  bottom: 0;
  width: 15%;
  background: color-mix(in srgb, var(--gc-loss) 45%, transparent);
}
.gc-rzband-r { right: 5%; }
.gc-rzband-l { left: 5%; }
.gc-fieldball {
